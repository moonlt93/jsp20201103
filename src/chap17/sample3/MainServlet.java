...
public class MainServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// list <- select sql
		List<Post> posts = getPosts(); // post 타입의 post 객체가 request객체에서 post정보를 검색 
										//get 메소드는 요청을 전송할때 필요한 데이터를 body에 담지 x 쿼리스트링을 통해 전송.
								 

		// request set attribute(list)
		request.setAttribute("posts", posts); //request객체에 posts객체에 있는 정보를 저장.

		// forward
		String path = "/WEB-INF/view/chap17/main.jsp";
		RequestDispatcher dispatcher = request.getRequestDispatcher(path);
		dispatcher.forward(request, response); // dispatcher에 담긴 정보를 
	}
